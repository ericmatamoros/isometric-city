    ...
    setInterval(async () => {
        try {
            ...
            const leaderboard = users.map(user => {
                ...
                .slice(0, 10)
                .map((entry, index) => (Object.assign(Object.assign({ rank: index + 1 }, entry), { isCurrentUser: false // Client will determine this
             })));
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("place_building", async (data) => {
            ...
            try {
                ...
                socket.emit("balance_update", updatedUser === null || updatedUser === void 0 ? void 0 : updatedUser.balance);
            }
            catch (e) {
        ...
        socket.on("buy_tile", async (data) => {
            ...
            try {
                ...
                io.to(worldId).emit("transaction_complete", {
                    ...
                    buyerBalance: updatedBuyer === null || updatedBuyer === void 0 ? void 0 : updatedBuyer.balance,
                    sellerAddress: (_a = tile.owner) === null || _a === void 0 ? void 0 : _a.walletAddress,
                    sellerBalance: updatedSeller === null || updatedSeller === void 0 ? void 0 : updatedSeller.balance,
                    x,
                    y,
                    price: tile.price
                });
        ...
        socket.on("collect_rent", async (data) => {
            ...
            try {
                ...
                if (tile.type.includes('commercial'))
                    rentAmount = 20;
                if (tile.type.includes('industrial'))
                    rentAmount = 30;
                if (tile.type.includes('large'))
                    rentAmount *= 2;
                if (tile.type.includes('skyscraper'))
                    rentAmount *= 5;
                // Execute transaction
                await prisma.$transaction([
                    prisma.user.update({
